...
export default function AdminFeedbackTable({ posts: initialPosts }: { posts: any[] }) {
  const [posts, setPosts] = useState(initialPosts);
  const [editingPostId, setEditingPostId] = useState<number | null>(null);
  const [postStatus, setPostStatus] = useState<Record<number, string>>(
    Object.fromEntries(initialPosts.map((post) => [post.id, post.status])),
  );
  ...
  const deleteFeedback = async (postId: number) => {
    // make sure they're not accidentally deleting
                    ...
                    {isEditing ? (
                      ...
                      <>
                        ...
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={() => deleteFeedback(post.id)}
                          className="gap-1 h-8 ml-2"
                        >
